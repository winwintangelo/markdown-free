...
const content: Partial<Record<Locale, {
  ...
  en: {
    ...
    filesProcessed: {
      ...
      preview: {
        title: "Preview, HTML, TXT, image, EPUB and Excel export",
        text: "When you upload a file for preview or export to HTML, TXT, PNG/JPG images, EPUB or Excel, everything happens entirely in your browser. Your file never leaves your device. We use client-side JavaScript to parse and render the Markdown. Remote images referenced by your document may be fetched through our image proxy so they can be embedded; the proxy sees the image URL, not your text."
      },
      pdf: {
        title: "PDF and Word (DOCX) export",
        text: "For PDF and Word (DOCX) generation, your Markdown content is sent to our server, converted, and immediately returned to you. The content is:",
        items: [
    ...
    analytics: {
      ...
      items: [
        ...
        "Counts of successful conversions (PDF, DOCX, PNG, EPUB, HTML, TXT)",
  ...
  it: {
    ...
    filesProcessed: {
      ...
      preview: {
        title: "Anteprima, esportazione HTML, TXT, immagine, EPUB ed Excel",
        text: "Quando carichi un file per l'anteprima o l'esportazione in HTML, TXT, immagine (PNG/JPG), EPUB o Excel, tutto avviene interamente nel tuo browser. Il tuo file non lascia mai il tuo dispositivo. Utilizziamo JavaScript lato client per analizzare e renderizzare il Markdown. Le immagini remote citate nel documento possono essere scaricate tramite il nostro proxy immagini per essere incorporate; il proxy vede l'URL dell'immagine, non il tuo testo."
      },
      pdf: {
        title: "Esportazione PDF e Word (DOCX)",
        text: "Per la generazione di PDF e Word (DOCX), il contenuto Markdown viene inviato al nostro server, convertito e immediatamente restituito. Il contenuto è:",
        items: [
    ...
    analytics: {
      ...
      items: [
        ...
        "Conteggio delle conversioni riuscite (PDF, DOCX, PNG, EPUB, HTML, TXT)",
    ...
    changes: {
      ...
      date: "Ultimo aggiornamento: Settembre 2026"
    },
    contact: {
      title: "Contatti",
  ...
  es: {
    ...
    filesProcessed: {
      ...
      preview: {
        title: "Vista previa, exportación HTML, TXT, imagen, EPUB y Excel",
        text: "Cuando subes un archivo para vista previa o exportación a HTML, TXT, imagen (PNG/JPG), EPUB o Excel, todo sucede completamente en tu navegador. Tu archivo nunca sale de tu dispositivo. Usamos JavaScript del lado del cliente para analizar y renderizar el Markdown. Las imágenes remotas referenciadas en tu documento pueden descargarse a través de nuestro proxy de imágenes para incrustarlas; el proxy ve la URL de la imagen, no tu texto."
      },
      pdf: {
        title: "Exportación PDF y Word (DOCX)",
        text: "Para la generación de PDF y Word (DOCX), tu contenido Markdown se envía a nuestro servidor, se convierte y se te devuelve inmediatamente. El contenido es:",
        items: [
    ...
    analytics: {
      ...
      items: [
        ...
        "Conteo de conversiones exitosas (PDF, DOCX, PNG, EPUB, HTML, TXT)",
  ...
  ja: {
    ...
    filesProcessed: {
      title: "ファイルの処理方法",
      preview: {
        title: "プレビュー、HTML・TXT・画像・EPUB・Excelエクスポート",
        text: "プレビュー、HTML・TXT・画像（PNG/JPG）・EPUB・Excelへのエクスポート時、すべてブラウザ内で処理されます。ファイルはデバイスから離れることはありません。クライアントサイドのJavaScriptでMarkdownを解析・レンダリングします。文書内で参照されている外部画像は、埋め込みのために当社の画像プロキシ経由で取得されることがあります。プロキシが受け取るのは画像のURLのみで、本文は含まれません。"
      },
      pdf: {
        title: "PDF・Word（DOCX）エクスポート",
        text: "PDFおよびWord（DOCX）生成時、Markdownコンテンツはサーバーに送信され、変換後すぐに返されます。コンテンツは：",
        items: [
          "メモリ内でのみ処理",
          "ディスクに書き込まれない",
          "PDF生成後すぐに破棄",
    ...
    analytics: {
      ...
      items: [
        "ページビューと訪問者数",
        "リファラー情報（訪問元）",
        "変換成功数（PDF、DOCX、PNG、EPUB、HTML、TXT）",
        "一般的なデバイス/ブラウザ情報"
      ],
  ...
  ko: {
    ...
    filesProcessed: {
      title: "파일 처리 방법",
      preview: {
        title: "미리보기, HTML·TXT·이미지·EPUB·Excel 내보내기",
        text: "미리보기나 HTML, TXT, 이미지(PNG/JPG), EPUB, Excel로 내보내기 위해 파일을 업로드하면 모든 것이 브라우저 내에서 처리됩니다. 파일이 기기를 떠나지 않습니다. 클라이언트 측 JavaScript를 사용하여 Markdown을 파싱하고 렌더링합니다. 문서에서 참조하는 외부 이미지는 삽입을 위해 당사 이미지 프록시를 통해 가져올 수 있으며, 프록시는 이미지 URL만 볼 뿐 본문은 보지 않습니다."
      },
      pdf: {
        title: "PDF 및 Word(DOCX) 내보내기",
        text: "PDF 및 Word(DOCX) 생성 시 Markdown 콘텐츠가 서버로 전송되고, 변환된 후 즉시 반환됩니다. 콘텐츠는:",
        items: [
          "메모리에서만 처리됨",
          "디스크에 기록되지 않음",
          "PDF 생성 후 즉시 삭제됨",
    ...
    analytics: {
      ...
      items: [
        "페이지 조회수 및 방문자 수",
        "리퍼러 정보 (방문자 유입 경로)",
        "성공적인 변환 횟수 (PDF, DOCX, PNG, EPUB, HTML, TXT)",
        "일반적인 기기/브라우저 정보"
      ],
  ...
  "zh-Hans": {
    ...
    filesProcessed: {
      title: "文件处理方式",
      preview: {
        title: "预览、HTML、TXT、图片、EPUB 和 Excel 导出",
        text: "当您上传文件进行预览或导出为 HTML、TXT、图片（PNG/JPG）、EPUB 或 Excel 时，所有处理都在您的浏览器中完成。您的文件永远不会离开您的设备。我们使用客户端 JavaScript 来解析和渲染 Markdown。文档中引用的外部图片可能会通过我们的图片代理获取以便嵌入；代理只会看到图片 URL，不会看到您的文本。"
      },
      pdf: {
        title: "PDF 和 Word（DOCX）导出",
        text: "生成 PDF 和 Word（DOCX）时，您的 Markdown 内容会发送到我们的服务器，转换后立即返回给您。内容：",
        items: [
          "仅在内存中处理",
          "从不写入磁盘",
          "PDF 生成后立即丢弃",
    ...
    analytics: {
      ...
      items: [
        "页面浏览量和访客数量",
        "来源信息（访客从哪里来）",
        "成功转换次数（PDF、DOCX、PNG、EPUB、HTML、TXT）",
        "一般设备/浏览器信息"
      ],
  ...
  "zh-Hant": {
    ...
    filesProcessed: {
      title: "檔案處理方式",
      preview: {
        title: "預覽、HTML、TXT、圖片、EPUB 和 Excel 匯出",
        text: "當您上傳檔案進行預覽或匯出為 HTML、TXT、圖片（PNG/JPG）、EPUB 或 Excel 時，所有處理都在您的瀏覽器中完成。您的檔案永遠不會離開您的裝置。我們使用客戶端 JavaScript 來解析和渲染 Markdown。文件中引用的外部圖片可能會透過我們的圖片代理取得以便嵌入；代理只會看到圖片 URL，不會看到您的文字。"
      },
      pdf: {
        title: "PDF 和 Word（DOCX）匯出",
        text: "生成 PDF 和 Word（DOCX）時，您的 Markdown 內容會傳送到我們的伺服器，轉換後立即回傳給您。內容：",
        items: [
          "僅在記憶體中處理",
          "從不寫入磁碟",
          "PDF 生成後立即丟棄",
    ...
    analytics: {
      ...
      items: [
        "頁面瀏覽量和訪客數量",
        "來源資訊（訪客從哪裡來）",
        "成功轉換次數（PDF、DOCX、PNG、EPUB、HTML、TXT）",
        "一般裝置/瀏覽器資訊"
      ],
  ...
  id: {
    ...
    filesProcessed: {
      ...
      preview: {
        title: "Pratinjau, ekspor HTML, TXT, gambar, EPUB & Excel",
        text: "Saat Anda mengunggah file untuk pratinjau atau ekspor ke HTML, TXT, gambar (PNG/JPG), EPUB, atau Excel, semuanya terjadi sepenuhnya di browser Anda. File Anda tidak pernah meninggalkan perangkat. Kami menggunakan JavaScript sisi klien untuk mengurai dan merender Markdown. Gambar jarak jauh yang dirujuk dokumen Anda dapat diambil melalui proxy gambar kami agar bisa disematkan; proxy hanya melihat URL gambar, bukan teks Anda."
      },
      pdf: {
        title: "Ekspor PDF & Word (DOCX)",
        text: "Untuk pembuatan PDF dan Word (DOCX), konten Markdown Anda dikirim ke server kami, dikonversi, dan segera dikembalikan kepada Anda. Konten tersebut:",
        items: [
    ...
    analytics: {
      ...
      items: [
        ...
        "Jumlah konversi yang berhasil (PDF, DOCX, PNG, EPUB, HTML, TXT)",
  ...
  vi: {
    ...
    filesProcessed: {
      ...
      preview: {
        title: "Xem trước, xuất HTML, TXT, ảnh, EPUB & Excel",
        text: "Khi bạn tải lên tệp để xem trước hoặc xuất sang HTML, TXT, ảnh (PNG/JPG), EPUB hoặc Excel, mọi thứ đều xảy ra hoàn toàn trong trình duyệt của bạn. Tệp của bạn không bao giờ rời khỏi thiết bị. Chúng tôi sử dụng JavaScript phía máy khách để phân tích cú pháp và hiển thị Markdown. Ảnh từ xa được tham chiếu trong tài liệu có thể được tải qua proxy ảnh của chúng tôi để nhúng; proxy chỉ thấy URL ảnh, không thấy văn bản của bạn."
      },
      pdf: {
        title: "Xuất PDF & Word (DOCX)",
        text: "Để tạo PDF và Word (DOCX), nội dung Markdown của bạn được gửi đến máy chủ của chúng tôi, chuyển đổi và ngay lập tức được trả về cho bạn. Nội dung:",
        items: [
    ...
    analytics: {
      ...
      items: [
        ...
        "Số lần chuyển đổi thành công (PDF, DOCX, PNG, EPUB, HTML, TXT)",
    ...
    changes: {
      ...
      date: "Cập nhật lần cuối: Tháng 9 năm 2026"
    },
    contact: {
      title: "Liên hệ",
