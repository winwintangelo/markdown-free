...
import { DATA_DIR, SnapshotStore, ensureDir, writeJsonAtomic, isMain } from './lib.mjs';
...
export function buildModel({ snapshot, measured = [], analysis = {}, discovery = {}, proposal = {}, prev = null }) {
  ...
  return {
    ...
    slate: proposal.portfolio?.slate || [], totalCandidates: proposal.portfolio?.totalCandidates || 0, issues,
  };
}
...
async function main() {
  ...
  const { mdPath, htmlPath } = generateReport({ snapshot, prev, analysis, proposal, discovery, measured: [] });
