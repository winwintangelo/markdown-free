...
export function buildModel({ snapshot, measured = [], analysis = {}, discovery = {}, proposal = {}, prev = null, market = {}, seo = null }) {
  ...
  (seo?.errors || []).forEach((e) => issues.push(`SEO: ${e.msg} — \`${e.path}\``));
...
export function toMarkdown(m) {
  ...
  if (m.seo) {
    L.push('');
    L.push('## 🔎 SEO hygiene');
    if (m.seo.ok) L.push(`_✅ ${m.seo.checked} pages checked (${m.seo.scope}) — no errors · ${m.seo.warnCount} warning(s)_`);
    else {
      L.push(`_❌ ${m.seo.errors.length} error(s) across ${m.seo.checked} pages (${m.seo.scope}) · ${m.seo.warnCount} warning(s):_`);
      m.seo.errors.slice(0, 12).forEach((e) => L.push(`- ❌ \`${e.path}\` — [${e.code}] ${e.msg}`));
    }
  }
  L.push('');
  L.push('---');
