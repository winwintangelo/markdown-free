...
export default defineConfig({
  testDir: "./e2e",
  testMatch: ["app.spec.ts", "i18n.spec.ts", "filename.spec.ts", "multilingual.spec.ts", "security.spec.ts", "word-pages.spec.ts", "share.spec.ts", "mobile.spec.ts", "clipboard.spec.ts", "related-tools.spec.ts", "image-export.spec.ts", "img-proxy.spec.ts", "comparison-inline.spec.ts", "feedback.spec.ts", "static-rendering.spec.ts", "fidelity.spec.ts", "intent-pages.spec.ts", "server-formats.spec.ts"], // Local tests (use production config for production.spec.ts)
